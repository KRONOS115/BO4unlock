...
std::uintptr_t processbase = reinterpret_cast<uintptr_t>(GetModuleHandleA(NULL));

const static auto Cbuf_AddText = reinterpret_cast<std::uintptr_t(__fastcall*)(int, const char*)>(processbase + 0x3CDE880);
		...
		do
		{
			if (te32.th32OwnerProcessID == GetCurrentProcessId() && te32.th32ThreadID != GetCurrentThreadId())
			{
				...
				if (hThread)
				{
					...
					{
						context.Dr0 = (uintptr_t)(processbase + 0x37F6ED0); //BM Spoof
...
LONG CALLBACK hookHandler(LPEXCEPTION_POINTERS ex)
{
	...
	{
		if (ex->ContextRecord->Rip == (uintptr_t)(processbase + 0x37F6ED0))
		{
